...
REGISTRY: dict[str, SettingSpec] = {
    ...
    "bo.exec.enabled": SettingSpec(
        ...
        edit_role="admin",
        sensitivity="normal",
        effect_ro="Pornit: POST /bo/execution/runs acceptă rulări; oprit: totul e respins cu EXEC_DISABLED.",
        acceptance_ro="Oprit implicit; rulările noi sunt refuzate cât e oprit.",
    ...
    "bo.exec.guardian_policy_secret_ref": SettingSpec(
        ...
        help_ro="Numele variabilei de mediu cu un credențial execpolicy:read — activează verificarea drepturilor efective (politica curentă) la fiecare efect. Gol de facto dacă variabila nu e setată.",
        owner_role="admin",
        edit_role="admin",
        sensitivity="normal",
        effect_ro="Setat + variabilă prezentă: la fiecare frontieră de efect se re-verifică politica curentă (revocată sau restrânsă → efect blocat). Lipsă → doar starea mandatului e verificată (limită documentată).",
